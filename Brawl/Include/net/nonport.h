...
s32 recvfrom(int socket, void* buffer, u32 length, int flags, struct sockaddr* address, u32* address_len);
s32 sendto(int socket, const void* message, u32 length, int flags, const struct sockaddr* dest_addr, u32 dest_len);
bool CanReceiveOnSocket(int socket);
bool CanSendOnSocket(int socket);

s32 recv(int sockfd, void* buf, size_t len, int flags);
s32 send(int sockfd, const void* buf, size_t len, int flags);
